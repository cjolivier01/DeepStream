...
void
count_output (GstElement *, GstBuffer *, GstPad *, gpointer user_data)
{
...
main (int argc, char **argv)
{
  ...
  if (pipeline == nullptr || source == nullptr || candidate == nullptr
  ...
  gst_bin_add_many (GST_BIN (pipeline), source, candidate, filter, sink,
                    nullptr);
  ...
  for (guint index = 0; index < frames; ++index) {
    ...
    if (gst_app_src_push_buffer (GST_APP_SRC (source), buffer)
        != GST_FLOW_OK) {
