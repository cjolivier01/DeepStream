...
void
set_error (gchar **error_message, const gchar *format, ...)
{
...
bool
validate_nvmm_descriptor (const GstMapInfo &map, const gchar *role,
                          bool require_filled, guint negotiated_batch_size,
                          guint *filled, gchar **error_message)
{
  if (map.data == nullptr || map.size < sizeof (NvBufSurface)) {
    set_error (error_message,
               "%s NVMM buffer has %" G_GSIZE_FORMAT
               " bytes; need at least %zu for an NvBufSurface descriptor",
               role, map.size, sizeof (NvBufSurface));
    return false;
  }
  ...
  if (surface->numFilled > surface->batchSize) {
    set_error (error_message,
               "%s NvBufSurface has %u filled surfaces but capacity %u", role,
               surface->numFilled, surface->batchSize);
    return false;
  }
  if (require_filled && surface->numFilled == 0) {
    set_error (error_message, "%s NvBufSurface contains no filled surfaces",
               role);
    return false;
  }
  if (require_filled && surface->numFilled > negotiated_batch_size) {
    set_error (error_message,
               ...
               "capacity is %u",
               role, surface->numFilled, negotiated_batch_size);
    return false;
  }
  if (filled != nullptr) {
...
yuv420_format (const GstVideoInfo &info)
{
  if (is_bt2020 (info)) {
    return NVBUF_COLOR_FORMAT_YUV420_2020;
  }
  if (is_bt709 (info)) {
    return is_full_range (info) ? NVBUF_COLOR_FORMAT_YUV420_709_ER
                                : NVBUF_COLOR_FORMAT_YUV420_709;
  }
  return is_full_range (info) ? NVBUF_COLOR_FORMAT_YUV420_ER
                              : NVBUF_COLOR_FORMAT_YUV420;
}

NvBufSurfaceColorFormat
nv12_format (const GstVideoInfo &info)
{
  if (is_bt2020 (info)) {
    return NVBUF_COLOR_FORMAT_NV12_2020;
  }
  if (is_bt709 (info)) {
    return is_full_range (info) ? NVBUF_COLOR_FORMAT_NV12_709_ER
                                : NVBUF_COLOR_FORMAT_NV12_709;
  }
  return is_full_range (info) ? NVBUF_COLOR_FORMAT_NV12_ER
                              : NVBUF_COLOR_FORMAT_NV12;
}

NvBufSurfaceColorFormat
p010_format (const GstVideoInfo &info)
{
  if (is_bt2020 (info)) {
    return NVBUF_COLOR_FORMAT_NV12_10LE_2020;
  }
  if (is_bt709 (info)) {
    return is_full_range (info) ? NVBUF_COLOR_FORMAT_NV12_10LE_709_ER
                                : NVBUF_COLOR_FORMAT_NV12_10LE_709;
  }
  return is_full_range (info) ? NVBUF_COLOR_FORMAT_NV12_10LE_ER
                              : NVBUF_COLOR_FORMAT_NV12_10LE;
}

NvBufSurfaceColorFormat
nv12_12_format (const GstVideoInfo &info)
{
  if (is_bt2020 (info)) {
    return NVBUF_COLOR_FORMAT_NV12_12LE_2020;
  }
  if (is_bt709 (info)) {
    return is_full_range (info) ? NVBUF_COLOR_FORMAT_NV12_12LE_709_ER
                                : NVBUF_COLOR_FORMAT_NV12_12LE_709;
  }
  return is_full_range (info) ? NVBUF_COLOR_FORMAT_NV12_12LE_ER
                              : NVBUF_COLOR_FORMAT_NV12_12LE;
}

NvBufSurfaceColorFormat
y444_format (const GstVideoInfo &info, guint bits)
{
  if (bits == 10) {
    if (is_bt2020 (info)) {
    ...
    if (is_bt709 (info)) {
      return is_full_range (info) ? NVBUF_COLOR_FORMAT_YUV444_10LE_709_ER
                                  : NVBUF_COLOR_FORMAT_YUV444_10LE_709;
    }
    return is_full_range (info) ? NVBUF_COLOR_FORMAT_YUV444_10LE_ER
  ...
  if (bits == 12) {
    ...
    if (is_bt709 (info)) {
      return is_full_range (info) ? NVBUF_COLOR_FORMAT_YUV444_12LE_709_ER
                                  : NVBUF_COLOR_FORMAT_YUV444_12LE_709;
    }
    return is_full_range (info) ? NVBUF_COLOR_FORMAT_YUV444_12LE_ER
  ...
  if (is_bt709 (info)) {
    return is_full_range (info) ? NVBUF_COLOR_FORMAT_YUV444_709_ER
                                : NVBUF_COLOR_FORMAT_YUV444_709;
  }
  return is_full_range (info) ? NVBUF_COLOR_FORMAT_YUV444_ER
                              : NVBUF_COLOR_FORMAT_YUV444;
}

NvBufSurfaceColorFormat
uyvy_format (const GstVideoInfo &info)
{
  if (is_bt2020 (info)) {
    return NVBUF_COLOR_FORMAT_UYVY_2020;
  }
  if (is_bt709 (info)) {
    return is_full_range (info) ? NVBUF_COLOR_FORMAT_UYVY_709_ER
                                : NVBUF_COLOR_FORMAT_UYVY_709;
  }
  return is_full_range (info) ? NVBUF_COLOR_FORMAT_UYVY_ER
                              : NVBUF_COLOR_FORMAT_UYVY;
}

NvBufSurfaceColorFormat
uyvp_format (const GstVideoInfo &info)
{
  if (is_bt2020 (info)) {
    return NVBUF_COLOR_FORMAT_UYVP_2020;
  }
  if (is_bt709 (info)) {
    return is_full_range (info) ? NVBUF_COLOR_FORMAT_UYVP_709_ER
                                : NVBUF_COLOR_FORMAT_UYVP_709;
  }
  return is_full_range (info) ? NVBUF_COLOR_FORMAT_UYVP_ER
                              : NVBUF_COLOR_FORMAT_UYVP;
}

NvBufSurfaceColorFormat
yuy2_format (const GstVideoInfo &info)
{
  return is_full_range (info) ? NVBUF_COLOR_FORMAT_YUYV_ER
                              : NVBUF_COLOR_FORMAT_YUYV;
}

NvBufSurfaceColorFormat
yvyu_format (const GstVideoInfo &info)
{
  return is_full_range (info) ? NVBUF_COLOR_FORMAT_YVYU_ER
                              : NVBUF_COLOR_FORMAT_YVYU;
}

NvBufSurfTransform_Flip
flip_method (guint method)
{
  switch (method) {
  case 1:
...
guint
raw_plane_rows (const GstVideoInfo &info, guint plane)
{
  for (guint component = 0;
       component < GST_VIDEO_INFO_N_COMPONENTS (&info); ++component) {
...
guint
raw_visible_row_bytes (const GstVideoInfo &info, guint plane)
{
  ...
  for (guint component = 0;
       component < GST_VIDEO_INFO_N_COMPONENTS (&info); ++component) {
    if (GST_VIDEO_INFO_COMP_PLANE (&info, component) == plane) {
      return GST_VIDEO_INFO_COMP_WIDTH (&info, component)
             * GST_VIDEO_INFO_COMP_PSTRIDE (&info, component);
    }
  }
  return 0;
}
...
bool
raw_plane_in_bounds (gsize size, gsize frame_offset, gsize plane_offset,
                     gint stride, guint rows, guint row_bytes)
{
  if (rows == 0 || row_bytes == 0 || frame_offset > size
      || plane_offset > size - frame_offset) {
    return false;
  }
  const gint64 first = static_cast<gint64> (frame_offset + plane_offset);
  const gint64 last
      = first + static_cast<gint64> (rows - 1) * stride;
  const gint64 lowest = std::min (first, last);
  const gint64 highest = std::max (first, last);
  return lowest >= 0
         && static_cast<guint64> (highest) + row_bytes <= size;
}

bool
mapped_plane_in_bounds (const GstMapInfo &map, const guint8 *data, gint stride,
                        guint rows, guint row_bytes)
{
  ...
  const std::uintptr_t map_address
      = reinterpret_cast<std::uintptr_t> (map.data);
  const std::uintptr_t data_address
      = reinterpret_cast<std::uintptr_t> (data);
  if (data_address < map_address
      || data_address - map_address > map.size) {
    return false;
  }
  const gint64 first
      = static_cast<gint64> (data_address - map_address);
  const gint64 last
      = first + static_cast<gint64> (rows - 1) * stride;
  const gint64 lowest = std::min (first, last);
  const gint64 highest = std::max (first, last);
  return lowest >= 0
         && static_cast<guint64> (highest) + row_bytes <= map.size;
}

class RawBufferMapping
{
public:
  ~RawBufferMapping () { unmap (); }
  ...
  bool
  map (GstBuffer *buffer, const GstVideoInfo &info, guint batch_size,
       GstMapFlags flags, const gchar *role, gchar **error_message)
  {
    info_ = &info;
    buffer_ = buffer;
    video_meta_ = batch_size == 1 ? gst_buffer_get_video_meta (buffer)
                                  : nullptr;
    if (video_meta_ != nullptr) {
      if (video_meta_->format != GST_VIDEO_INFO_FORMAT (&info)
          || video_meta_->width
                 != static_cast<guint> (GST_VIDEO_INFO_WIDTH (&info))
          || video_meta_->height
                 != static_cast<guint> (GST_VIDEO_INFO_HEIGHT (&info))
          || video_meta_->n_planes < GST_VIDEO_INFO_N_PLANES (&info)) {
        set_error (error_message,
                   "%s GstVideoMeta does not match negotiated video layout",
                   role);
        return false;
      }
      for (guint plane = 0; plane < GST_VIDEO_INFO_N_PLANES (&info); ++plane) {
        ...
        gint stride = 0;
        if (!gst_video_meta_map (video_meta_, plane, &plane_maps_[plane],
                                 &data, &stride, flags)) {
          set_error (error_message, "failed to map %s video plane %u", role,
                     plane);
          return false;
        }
    ...
    buffer_mapped_ = true;
    if (info.size == 0 || batch_size > G_MAXSIZE / info.size
        || buffer_map_.size < info.size * batch_size) {
      set_error (error_message,
                 "%s buffer has %" G_GSIZE_FORMAT
                 " bytes; need at least %" G_GSIZE_FORMAT,
                 role, buffer_map_.size,
                 batch_size > G_MAXSIZE / std::max<gsize> (1, info.size)
                     ? G_MAXSIZE
  ...
  guint8 *
  plane_data (guint batch, guint plane) const
  {
    if (video_meta_ != nullptr) {
      return plane_data_[plane];
    }
    return buffer_map_.data + batch * info_->size
           + GST_VIDEO_INFO_PLANE_OFFSET (info_, plane);
  }

  gint
  plane_stride (guint plane) const
  {
    return video_meta_ != nullptr
               ? plane_stride_[plane]
               : GST_VIDEO_INFO_PLANE_STRIDE (info_, plane);
  }

  bool
  plane_in_bounds (guint batch, guint plane, guint rows,
                   guint row_bytes) const
  {
    const gint stride = plane_stride (plane);
    if (video_meta_ != nullptr) {
      return mapped_plane_in_bounds (plane_maps_[plane], plane_data_[plane],
                                     stride, rows, row_bytes);
    }
    return raw_plane_in_bounds (
        buffer_map_.size, batch * info_->size,
        GST_VIDEO_INFO_PLANE_OFFSET (info_, plane), stride, rows, row_bytes);
  }
...
bool
copy_raw_to_surface (const GstVideoInfo &info, GstBuffer *buffer,
                     NvBufSurface *surface, guint batch_size,
                     gchar **error_message)
{
  RawBufferMapping raw;
  if (!raw.map (buffer, info, batch_size, GST_MAP_READ, "raw input",
                error_message)) {
    return false;
  }

  const guint raw_planes = GST_VIDEO_INFO_N_PLANES (&info);
  const bool map_surface = surface->memType == NVBUF_MEM_SURFACE_ARRAY
  ...
  for (guint batch = 0; batch < batch_size; ++batch) {
    ...
    if (map_surface
        && NvBufSurfaceMap (surface, batch, -1, NVBUF_MAP_WRITE) != 0) {
    ...
    for (guint raw_plane = 0; raw_plane < raw_planes; ++raw_plane) {
      ...
      const guint absolute_stride = static_cast<guint> (
          std::abs (static_cast<gint64> (raw_stride)));
      const guint visible_row_bytes
          = raw_visible_row_bytes (info, raw_plane);
      ...
      if (raw_stride == 0 || absolute_stride < visible_row_bytes) {
        set_error (error_message,
                   ...
                   raw_plane, raw_stride, visible_row_bytes);
        batch_success = false;
        break;
      }
      if (params.planeParams.height[surface_plane] < rows
          || surface_pitch < visible_row_bytes) {
        set_error (error_message,
                   "raw input plane %u exceeds the staging surface layout",
                   raw_plane);
        batch_success = false;
        break;
      }
      if (!raw.plane_in_bounds (batch, raw_plane, rows,
                                visible_row_bytes)) {
      ...
      guint8 *destination
          = map_surface
                ...
                : static_cast<guint8 *> (params.dataPtr)
                      + params.planeParams.offset[surface_plane];
      ...
      if (map_surface || raw_stride < 0) {
        for (guint row = 0; row < rows; ++row) {
          ...
          if (map_surface) {
            std::memcpy (destination + row * surface_pitch, source_row,
                         visible_row_bytes);
          } else {
            const cudaError_t copy_error
                = cudaMemcpy (destination + row * surface_pitch, source_row,
                              visible_row_bytes, cudaMemcpyHostToDevice);
            if (copy_error != cudaSuccess) {
              set_error (error_message, "raw upload failed: %s",
        ...
        const cudaError_t copy_error
            = cudaMemcpy2D (destination, surface_pitch, source, raw_stride,
                            visible_row_bytes, rows, cudaMemcpyHostToDevice);
        if (copy_error != cudaSuccess) {
          set_error (error_message, "raw upload failed: %s",
                     cudaGetErrorString (copy_error));
          batch_success = false;
    ...
    if (map_surface && batch_success
        && NvBufSurfaceSyncForDevice (surface, batch, -1) != 0) {
      set_error (error_message, "failed to sync raw-input staging surface");
      batch_success = false;
    }
    if (map_surface) {
...
bool
copy_surface_to_raw (const NvBufSurface *surface, const GstVideoInfo &info,
                     GstBuffer *buffer, guint batch_size,
                     gchar **error_message)
{
  RawBufferMapping raw;
  if (!raw.map (buffer, info, batch_size, GST_MAP_WRITE, "raw output",
                error_message)) {
    return false;
  }
  ...
  for (guint batch = 0; batch < batch_size; ++batch) {
    ...
    if (map_surface
        && NvBufSurfaceMap (writable_surface, batch, -1, NVBUF_MAP_READ)
               != 0) {
      set_error (error_message, "failed to map raw-output staging surface");
      return false;
    }
    if (map_surface
        && NvBufSurfaceSyncForCpu (writable_surface, batch, -1) != 0) {
      NvBufSurfaceUnMap (writable_surface, batch, -1);
      set_error (error_message, "failed to sync raw-output staging surface");
      return false;
    }
    for (guint raw_plane = 0; raw_plane < raw_planes; ++raw_plane) {
      const guint surface_plane = raw_plane;
      ...
      if (params.planeParams.height[surface_plane] < rows
          || surface_pitch < visible_row_bytes) {
        set_error (error_message,
                   "raw output plane %u exceeds the staging surface layout",
                   raw_plane);
        batch_success = false;
        break;
      }
      const guint8 *source
          = map_surface ? static_cast<const guint8 *> (
                              params.mappedAddr.addr[surface_plane])
                        : static_cast<const guint8 *> (params.dataPtr)
                              + params.planeParams.offset[surface_plane];
      if (source == nullptr) {
      ...
      if (map_surface || raw_stride < 0) {
        for (guint row = 0; row < rows; ++row) {
          guint8 *destination_row
              = destination + static_cast<gint64> (row) * raw_stride;
          if (map_surface) {
            std::memcpy (destination_row, source + row * surface_pitch,
                         visible_row_bytes);
          } else {
            const cudaError_t copy_error
                = cudaMemcpy (destination_row, source + row * surface_pitch,
                              visible_row_bytes, cudaMemcpyDeviceToHost);
            if (copy_error != cudaSuccess) {
              set_error (error_message, "raw download failed: %s",
                         cudaGetErrorString (copy_error));
              batch_success = false;
              break;
            }
        ...
        const cudaError_t copy_error
            = cudaMemcpy2D (destination, raw_stride, source, surface_pitch,
                            visible_row_bytes, rows, cudaMemcpyDeviceToHost);
        if (copy_error != cudaSuccess) {
          set_error (error_message, "raw download failed: %s",
                     cudaGetErrorString (copy_error));
          batch_success = false;
...
NvBufSurfaceColorFormat
dsx_video_info_to_nvbuf_format (const GstVideoInfo &info)
{
  switch (GST_VIDEO_INFO_FORMAT (&info)) {
  case GST_VIDEO_FORMAT_I420:
  ...
  case GST_VIDEO_FORMAT_GRAY8:
    return is_full_range (info) ? NVBUF_COLOR_FORMAT_GRAY8_ER
  ...
  case GST_VIDEO_FORMAT_BGR10A2_LE:
    return is_bt2020 (info) ? NVBUF_COLOR_FORMAT_BGRA_10_10_10_2_2020
                            : NVBUF_COLOR_FORMAT_BGRA_10_10_10_2_709;
  case GST_VIDEO_FORMAT_RGB10A2_LE:
    return is_bt2020 (info) ? NVBUF_COLOR_FORMAT_RGBA_10_10_10_2_2020
...
DsxVideoConvertBackend::~DsxVideoConvertBackend () { stop (); }

bool
DsxVideoConvertBackend::start (guint gpu_id, gchar **error_message)
{
  if (started_ && gpu_id != gpu_id_) {
    set_error (error_message,
               "cannot change GPU from %u to %u while the backend is active",
               gpu_id_, gpu_id);
    return false;
  }
  const cudaError_t select_error
      = cudaSetDevice (static_cast<int> (gpu_id));
  if (select_error != cudaSuccess) {
    set_error (error_message, "cudaSetDevice(%u) failed: %s", gpu_id,
  ...
  if (cuda_error != cudaSuccess) {
    set_error (error_message, "CUDA stream creation failed: %s",
...
bool
DsxVideoConvertBackend::configure (const GstVideoInfo &input_info,
                                   const GstVideoInfo &output_info,
                                   bool input_nvmm, bool output_nvmm,
                                   guint batch_size,
                                   ...
                                   gchar **error_message)
{
  ...
  if (input_format_ == NVBUF_COLOR_FORMAT_INVALID
      || output_format_ == NVBUF_COLOR_FORMAT_INVALID) {
    set_error (error_message, "unsupported negotiated video format");
    return false;
  }
  NvBufSurfTransformConfigParams session{};
  session.compute_mode
      = static_cast<NvBufSurfTransform_Compute> (options_.compute_hw);
  session.gpu_id = static_cast<int32_t> (options_.gpu_id);
  session.cuda_stream = stream_;
  if (NvBufSurfTransformSetSessionParams (&session)
...
bool
DsxVideoConvertBackend::ensure_staging_surfaces (guint batch_size,
                                                 gchar **error_message)
{
  const cudaError_t select_error
      = cudaSetDevice (static_cast<int> (options_.gpu_id));
  if (select_error != cudaSuccess) {
    set_error (error_message, "cudaSetDevice(%u) failed: %s",
               options_.gpu_id, cudaGetErrorString (select_error));
    return false;
  }
  if (!input_nvmm_
      && (input_staging_ == nullptr
          || input_staging_->batchSize != batch_size)) {
    if (input_staging_ != nullptr) {
      NvBufSurfaceDestroy (input_staging_);
      input_staging_ = nullptr;
    }
    ...
    params.memType = options_.copy_hw == 2 ? NVBUF_MEM_SURFACE_ARRAY
    ...
    if (NvBufSurfaceCreate (&input_staging_, batch_size, &params) != 0) {
      set_error (error_message,
                 "failed to allocate raw-input staging surface");
      return false;
    }
  }
  ...
  if (!output_nvmm_
      && (output_staging_ == nullptr
          || output_staging_->batchSize != batch_size)) {
    if (output_staging_ != nullptr) {
      NvBufSurfaceDestroy (output_staging_);
      output_staging_ = nullptr;
    }
    ...
    if (NvBufSurfaceCreate (&output_staging_, batch_size, &params) != 0) {
      set_error (error_message,
                 "failed to allocate raw-output staging surface");
      return false;
    }
  }
  return true;
...
bool
DsxVideoConvertBackend::upload_raw (GstBuffer *buffer, NvBufSurface *surface,
                                    guint batch_size,
                                    gchar **error_message) const
{
  return copy_raw_to_surface (input_info_, buffer, surface, batch_size,
                              error_message);
}

bool
DsxVideoConvertBackend::download_raw (const NvBufSurface *surface,
                                      GstBuffer *buffer, guint batch_size,
                                      gchar **error_message) const
{
  return copy_surface_to_raw (surface, output_info_, buffer, batch_size,
                              error_message);
}

bool
DsxVideoConvertBackend::transform (GstBuffer *input, GstBuffer *output,
                                   ...
                                   gchar **error_message)
{
  ...
  if (output_nvmm_
      && !gst_buffer_map (output, &output_map, GST_MAP_READWRITE)) {
    if (input_mapped) {
  ...
  bool success = true;
  if (input_nvmm_) {
    success = validate_nvmm_descriptor (
        input_map, "input", true, batch_size_, &batch_size, error_message);
  }

  NvBufSurface *input_surface
      = input_nvmm_ ? reinterpret_cast<NvBufSurface *> (input_map.data)
                    : nullptr;
  if (success && input_nvmm_
      && (input_surface->memType == NVBUF_MEM_DEFAULT
          || input_surface->memType == NVBUF_MEM_CUDA_DEVICE)
      && input_surface->gpuId != options_.gpu_id) {
    set_error (error_message,
               "input surface GPU %u does not match configured GPU %u",
               input_surface->gpuId, options_.gpu_id);
    if (output_mapped) {
  ...
  NvBufSurface *output_surface = output_nvmm_
                                     ? reinterpret_cast<NvBufSurface *> (
                                           output_map.data)
                                     : output_staging_;
  if (success && output_nvmm_) {
    success = validate_nvmm_descriptor (output_map, "output", false,
                                        batch_size_, nullptr, error_message);
  }
  if (success && output_surface->batchSize < batch_size) {
    set_error (error_message,
               "output surface batch size %u is smaller than %u",
               output_surface->batchSize, batch_size);
    success = false;
  }
  ...
  if (success) {
    ...
    if (destination_crop.enabled ()) {
      if (NvBufSurfaceMemSet (output_surface, -1, -1, 0) != 0) {
        ...
        const guint planes
            = output_surface->surfaceList[0].planeParams.num_planes;
        for (guint plane = 1; plane < planes; ++plane) {
          if (NvBufSurfaceMemSet (output_surface, -1,
                                  static_cast<int> (plane), 128)
              != 0) {
            set_error (error_message,
                       "failed to initialize destination chroma plane");
            success = false;
            break;
          }
        }
    ...
    guint source_width
        = source_crop.enabled ()
              ? std::min (source_crop.width, input_width - source_left)
              : input_surface->surfaceList[0].planeParams.width[0];
    guint source_height
        = source_crop.enabled ()
              ? std::min (source_crop.height, input_height - source_top)
              : input_surface->surfaceList[0].planeParams.height[0];
    const guint destination_left
        = std::min (destination_crop.left, output_width - 1);
    const guint destination_top
        = std::min (destination_crop.top, output_height - 1);
    guint destination_width
        = destination_crop.enabled ()
              ? std::min (destination_crop.width,
                          output_width - destination_left)
              : output_surface->surfaceList[0].planeParams.width[0];
    guint destination_height
        = destination_crop.enabled ()
              ? std::min (destination_crop.height,
    ...
    if (source_width == 0 || source_height == 0 || destination_width == 0
      ...
      transform_params.transform_filter
          = static_cast<NvBufSurfTransform_Inter> (
              options_.interpolation_method);
      ...
      if (NvBufSurfTransformSetSessionParams (&session)
        ...
        const NvBufSurfTransform_Error result = NvBufSurfTransform (
            input_surface, output_surface, &transform_params);
        if (result != NvBufSurfTransformError_Success) {
          set_error (error_message, "NvBufSurfTransform failed with error %d",
