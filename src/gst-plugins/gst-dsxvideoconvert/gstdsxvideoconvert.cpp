...
#define PACKAGE_ORIGIN "https://github.com/NVIDIA/DeepStream"
...
#endif
#define DSX_CAPS                                                              \
  GST_VIDEO_CAPS_MAKE_WITH_FEATURES (GST_CAPS_FEATURE_MEMORY_NVMM,            \
                                     DSX_NVMM_FORMATS)                        \
...
static GstStaticPadTemplate sink_template = GST_STATIC_PAD_TEMPLATE (
    "sink", GST_PAD_SINK, GST_PAD_ALWAYS, GST_STATIC_CAPS (DSX_CAPS));
static GstStaticPadTemplate source_template = GST_STATIC_PAD_TEMPLATE (
    "src", GST_PAD_SRC, GST_PAD_ALWAYS, GST_STATIC_CAPS (DSX_CAPS));

G_DEFINE_TYPE (GstDsxVideoConvert, gst_dsx_video_convert,
...
dsx_flip_method_get_type ()
{
  ...
  static const GEnumValue values[] = {
    ...
    { 5, "Flip across upper right/lower left diagonal",
      "upper-right-diagonal" },
    { 6, "Flip vertically", "vertical-flip" },
    { 7, "Flip across upper left/lower right diagonal",
      "upper-left-diagonal" },
    { 0, nullptr, nullptr },
  };
  if (g_once_init_enter (&type)) {
    const GType registered
        = g_enum_register_static ("DsxVideoFlipMethod", values);
...
dsx_interpolation_method_get_type ()
{
  ...
  if (g_once_init_enter (&type)) {
    const GType registered
        = g_enum_register_static ("DsxInterpolationMethod", values);
    g_once_init_leave (&type, registered);
  }
  return static_cast<GType> (type);
}
...
dsx_compute_hw_get_type ()
{
  ...
  if (g_once_init_enter (&type)) {
    const GType registered
        = g_enum_register_static ("DsxComputeHWType", values);
    g_once_init_leave (&type, registered);
  }
  return static_cast<GType> (type);
}
...
dsx_memory_type_get_type ()
{
...
#if defined(__aarch64__) && !defined(AARCH64_IS_SBSA)
  static const GEnumValue values[] = {
    { 0, "Default memory allocated, specific to particular platform",
      "nvbuf-mem-default" },
    { 4, "Allocate Surface Array memory, applicable for Jetson",
      "nvbuf-mem-surface-array" },
    { 0, nullptr, nullptr },
  };
#else
  static const GEnumValue values[] = {
  ...
  if (g_once_init_enter (&type)) {
    const GType registered
        = g_enum_register_static ("DsxNvBufMemoryType", values);
    g_once_init_leave (&type, registered);
  }
  return static_cast<GType> (type);
}
...
bool
caps_are_nvmm (const GstCaps *caps)
{
...
bool
parse_crop (const gchar *text, DsxCropRect *crop)
{
  ...
  guint height = 0;
  gchar trailing = '\0';
  if (std::sscanf (text, "%u:%u:%u:%u%c", &left, &top, &width, &height,
                   &trailing)
      != 4) {
    return false;
  }
...
bool
is_bgra64_or_uyvp (GstVideoFormat format)
{
  return format == GST_VIDEO_FORMAT_BGRA64_LE
         || format == GST_VIDEO_FORMAT_UYVP;
}

bool
is_bgra64_pair_supported (GstVideoFormat input, GstVideoFormat output)
{
  if (input != GST_VIDEO_FORMAT_BGRA64_LE
      && output != GST_VIDEO_FORMAT_BGRA64_LE) {
    return true;
  }
...
bool
caps_batch_size (const GstStructure *structure, guint *batch_size)
{
...
guint
query_batch_size (GstDsxVideoConvert *self, GstCaps *input_caps)
{
...
DsxBackendOptions
backend_options (const GstDsxVideoConvert *self)
{
...
gfloat
scale_coordinate (gfloat value, guint crop_offset, gdouble scale,
                  guint destination_offset)
{
...
void
scale_rect (NvOSD_RectParams *rect, guint source_left, guint source_top,
            gdouble scale_x, gdouble scale_y, guint destination_left,
            guint destination_top)
{
  rect->left
      = scale_coordinate (rect->left, source_left, scale_x, destination_left);
  rect->top
      = scale_coordinate (rect->top, source_top, scale_y, destination_top);
  rect->width = static_cast<gfloat> (rect->width * scale_x);
  rect->height = static_cast<gfloat> (rect->height * scale_y);
}

void
scale_text (NvOSD_TextParams *text, guint source_left, guint source_top,
            gdouble scale_x, gdouble scale_y, guint destination_left,
            guint destination_top, bool scale_font)
{
  const gfloat scaled_x
      = (text->x_offset - source_left) * static_cast<gfloat> (scale_x)
        + destination_left;
  const gfloat scaled_y
      = (text->y_offset - source_top) * static_cast<gfloat> (scale_y)
        + destination_top;
  text->x_offset = static_cast<guint> (scaled_x);
  text->y_offset = static_cast<guint> (scaled_y);
  if (scale_font) {
    text->font_params.font_size = static_cast<guint> (
        text->font_params.font_size * std::min (scale_x, scale_y));
  }
}
...
MetadataTransform
metadata_transform (const GstDsxVideoConvert *self)
{
...
void
orient_object (NvDsObjectMeta *object, guint method, guint source_width,
               guint source_height)
{
  ...
  if (method != 0 && method != 4) {
    object->text_params.y_offset
        = static_cast<guint> (rect.top)
          - object->text_params.font_params.font_size;
  }
}

void
scale_metadata (const MetadataTransform &transform, GstBuffer *output)
{
  ...
  const guint source_left
      = transform.source_crop.enabled ()
            ? std::min (transform.source_crop.left, input_width - 1)
            : 0;
  const guint source_top
      = transform.source_crop.enabled ()
            ? std::min (transform.source_crop.top, input_height - 1)
            : 0;
  const guint source_width
      = transform.source_crop.enabled ()
            ? std::min (transform.source_crop.width, input_width - source_left)
            : input_width;
  const guint source_height
      = transform.source_crop.enabled ()
            ? std::min (transform.source_crop.height,
                        input_height - source_top)
            : input_height;
  const guint destination_left
      = transform.destination_crop.enabled ()
            ? std::min (transform.destination_crop.left, output_width - 1)
            : 0;
  const guint destination_top
      = transform.destination_crop.enabled ()
            ? std::min (transform.destination_crop.top, output_height - 1)
            : 0;
  const guint destination_width
      = transform.destination_crop.enabled ()
            ? std::min (transform.destination_crop.width,
                        output_width - destination_left)
            : output_width;
  const guint destination_height
      = transform.destination_crop.enabled ()
            ? std::min (transform.destination_crop.height,
                        output_height - destination_top)
            : output_height;
  if (source_width == 0 || source_height == 0) {
    return;
  }
  const bool swaps_axes = transform.flip_method == 1
                          || transform.flip_method == 3
                          || transform.flip_method == 5
                          || transform.flip_method == 7;
  const guint scale_source_width
      = swaps_axes ? source_height : source_width;
  const guint scale_source_height
      = swaps_axes ? source_width : source_height;
  const gdouble scale_x
      = static_cast<gdouble> (destination_width) / scale_source_width;
  const gdouble scale_y
      = static_cast<gdouble> (destination_height) / scale_source_height;
  ...
  for (NvDsMetaList *frame_node = batch_meta->frame_meta_list;
    ...
    for (NvDsMetaList *object_node = frame->obj_meta_list;
      ...
      orient_object (object, transform.flip_method, source_width,
                     source_height);
      scale_rect (&object->rect_params, source_left, source_top, scale_x,
                  scale_y, destination_left, destination_top);
      scale_text (&object->text_params, source_left, source_top, scale_x,
                  scale_y, destination_left, destination_top, true);
    }

    for (NvDsMetaList *display_node = frame->display_meta_list;
         display_node != nullptr; display_node = display_node->next) {
      auto *display = static_cast<NvDsDisplayMeta *> (display_node->data);
      for (guint index = 0; index < display->num_rects; ++index) {
        scale_rect (&display->rect_params[index], source_left, source_top,
                    scale_x, scale_y, destination_left, destination_top);
      }
      for (guint index = 0; index < display->num_labels; ++index) {
        scale_text (&display->text_params[index], source_left, source_top,
                    scale_x, scale_y, destination_left, destination_top,
                    false);
      }
      for (guint index = 0; index < display->num_lines; ++index) {
        NvOSD_LineParams &line = display->line_params[index];
        line.x1 = static_cast<guint> (scale_coordinate (
            line.x1, source_left, scale_x, destination_left));
        line.x2 = static_cast<guint> (scale_coordinate (
            line.x2, source_left, scale_x, destination_left));
        line.y1 = static_cast<guint> (
            scale_coordinate (line.y1, source_top, scale_y, destination_top));
        line.y2 = static_cast<guint> (
            scale_coordinate (line.y2, source_top, scale_y, destination_top));
      }
      for (guint index = 0; index < display->num_arrows; ++index) {
        NvOSD_ArrowParams &arrow = display->arrow_params[index];
        arrow.x1 = static_cast<guint> (scale_coordinate (
            arrow.x1, source_left, scale_x, destination_left));
        arrow.x2 = static_cast<guint> (scale_coordinate (
            arrow.x2, source_left, scale_x, destination_left));
        arrow.y1 = static_cast<guint> (
            scale_coordinate (arrow.y1, source_top, scale_y, destination_top));
        arrow.y2 = static_cast<guint> (
            scale_coordinate (arrow.y2, source_top, scale_y, destination_top));
      }
      for (guint index = 0; index < display->num_circles; ++index) {
        NvOSD_CircleParams &circle = display->circle_params[index];
        circle.xc = static_cast<guint> (scale_coordinate (
            circle.xc, source_left, scale_x, destination_left));
        circle.yc = static_cast<guint> (scale_coordinate (
            circle.yc, source_top, scale_y, destination_top));
        circle.radius
            = static_cast<guint> (std::lround (circle.radius * shape_scale));
      }
    }
  }
...
void
copy_fixed_field (GstStructure *destination, const GstStructure *source,
                  const gchar *field)
{
...
const gchar *
memory_type_caps_name (gint memory_type)
{
#if defined(__aarch64__) && !defined(AARCH64_IS_SBSA)
...
void
add_output_caps_fields (GstStructure *structure,
                        const GstDsxVideoConvert *self)
{
  gst_structure_set (
      structure, "block-linear", G_TYPE_BOOLEAN, self->block_linear_output,
      "nvbuf-memory-type", G_TYPE_STRING,
      memory_type_caps_name (self->nvbuf_memory_type), "gpu-id", G_TYPE_INT,
      static_cast<gint> (std::min (self->gpu_id,
                                   static_cast<guint> (G_MAXINT))),
      nullptr);
}
...
static void
gst_dsx_video_convert_set_property (GObject *object, guint property_id,
                                    const GValue *value, GParamSpec *parameter)
{
  ...
  case PROP_SOURCE_CROP: {
    ...
    if (!parse_crop (text, &crop)) {
      GST_WARNING_OBJECT (self, "invalid src-crop value '%s'",
  ...
  case PROP_DESTINATION_CROP: {
    ...
    const gchar *text = g_value_get_string (value);
    if (!parse_crop (text, &crop)) {
      GST_WARNING_OBJECT (self, "invalid dest-crop value '%s'",
                          text != nullptr ? text : "(null)");
      break;
    }
    g_mutex_lock (&self->lock);
...
static void
gst_dsx_video_convert_get_property (GObject *object, guint property_id,
                                    GValue *value, GParamSpec *parameter)
{
  GstDsxVideoConvert *self = GST_DSX_VIDEO_CONVERT (object);
...
static gboolean
gst_dsx_video_convert_start (GstBaseTransform *transform)
{
  ...
  if (!started) {
    GST_ELEMENT_ERROR (
        self, RESOURCE, FAILED, ("Failed to initialize GPU converter"),
...
static gboolean
gst_dsx_video_convert_stop (GstBaseTransform *transform)
{
  GstDsxVideoConvert *self = GST_DSX_VIDEO_CONVERT (transform);
  g_mutex_lock (&self->lock);
  self->backend->stop ();
...
static gboolean
gst_dsx_video_convert_set_caps (GstBaseTransform *transform,
                                GstCaps *input_caps, GstCaps *output_caps)
{
  ...
  const GstStructure *input_structure
      = gst_caps_get_structure (input_caps, 0);
  GstStructure *output_structure
      = gst_caps_get_structure (effective_output_caps, 0);
  ...
  copy_fixed_field (output_structure, input_structure,
  ...
  if (!is_bgra64_pair_supported (GST_VIDEO_INFO_FORMAT (&input_info),
                                 GST_VIDEO_INFO_FORMAT (&output_info))) {
    GST_ERROR_OBJECT (self,
                      "BGRA64_LE is supported only with BGRA64_LE or UYVP");
    gst_caps_unref (effective_output_caps);
    return FALSE;
  }
  ...
  const guint batch_size
      = input_nvmm ? query_batch_size (self, input_caps) : 1;
  ...
  const gboolean needs_transform = self->disable_passthrough
                                   || self->source_crop.enabled ()
                                   || self->destination_crop.enabled ()
                                   || self->flip_method != 0 || !same_caps;
  const bool configured
      = !needs_transform
        || self->backend->configure (
            self->input_info, self->output_info, self->input_nvmm,
            self->output_nvmm, self->batch_size, backend_options (self),
            &error_message);
  g_mutex_unlock (&self->lock);
  const gboolean caps_updated
      = gst_caps_is_equal (output_caps, effective_output_caps)
        || gst_base_transform_update_src_caps (transform,
  ...
  if (!configured) {
    GST_ERROR_OBJECT (self, "converter configuration failed: %s",
...
static GstCaps *
gst_dsx_video_convert_transform_caps (GstBaseTransform *transform,
                                      GstPadDirection direction, GstCaps *caps,
                                      GstCaps *filter)
{
  GstPadTemplate *target_template
      = direction == GST_PAD_SINK
            ? gst_element_class_get_pad_template (
                  GST_ELEMENT_GET_CLASS (transform), "src")
            : gst_element_class_get_pad_template (
                  GST_ELEMENT_GET_CLASS (transform), "sink");
...
static gboolean
gst_dsx_video_convert_accept_caps (GstBaseTransform *transform,
                                   GstPadDirection direction, GstCaps *caps)
{
...
static GstCaps *
gst_dsx_video_convert_fixate_caps (GstBaseTransform *transform,
                                   GstPadDirection direction, GstCaps *caps,
                                   GstCaps *other_caps)
{
  GstDsxVideoConvert *self = GST_DSX_VIDEO_CONVERT (transform);
  ...
  if (flip_method == 1 || flip_method == 3 || flip_method == 5
      || flip_method == 7) {
  ...
  GST_LOG_OBJECT (transform,
                  "fixated %" GST_PTR_FORMAT " against %" GST_PTR_FORMAT
                  " in direction %s",
                  other_caps, caps,
...
static gboolean
gst_dsx_video_convert_get_unit_size (GstBaseTransform *, GstCaps *caps,
                                     gsize *size)
{
...
static gboolean
gst_dsx_video_convert_transform_size (GstBaseTransform *transform,
                                      GstPadDirection, GstCaps *, gsize,
                                      GstCaps *other_caps, gsize *other_size)
{
  return gst_dsx_video_convert_get_unit_size (transform, other_caps,
                                              other_size);
}

static gboolean
gst_dsx_video_convert_transform_meta (GstBaseTransform *, GstBuffer *,
...
static gboolean
gst_dsx_video_convert_decide_allocation (GstBaseTransform *transform,
                                         GstQuery *query)
{
  GstDsxVideoConvert *self = GST_DSX_VIDEO_CONVERT (transform);
  g_mutex_lock (&self->lock);
  ...
  gst_buffer_pool_config_set_params (configuration, caps, buffer_size,
                                     output_buffers, output_buffers);
  gst_structure_set (
      configuration, "memtype", G_TYPE_UINT, memory_type, "gpu-id",
      G_TYPE_UINT, gpu_id, "batch-size", G_TYPE_UINT, batch_size,
      "bl-output", G_TYPE_UINT, block_linear_output ? 1U : 0U,
      "contiguous-alloc", G_TYPE_BOOLEAN, contiguous_buffers,
      "clear-chroma", G_TYPE_BOOLEAN, TRUE, nullptr);
...
gst_dsx_video_convert_prepare_output_buffer (GstBaseTransform *transform,
                                             GstBuffer *input,
                                             GstBuffer **output)
{
  GstDsxVideoConvert *self = GST_DSX_VIDEO_CONVERT (transform);
  g_mutex_lock (&self->lock);
  const gboolean needs_batched_raw
      = !self->output_nvmm && self->batch_size > 1;
...
static GstFlowReturn
gst_dsx_video_convert_transform (GstBaseTransform *transform, GstBuffer *input,
                                 GstBuffer *output)
{
  GstDsxVideoConvert *self = GST_DSX_VIDEO_CONVERT (transform);
  gchar *error_message = nullptr;
  ...
  g_mutex_lock (&self->lock);
  const bool success
      = self->backend->transform (input, output, self->source_crop,
                                  self->destination_crop, &error_message);
  const MetadataTransform metadata = metadata_transform (self);
  const gboolean silent = self->silent;
  g_mutex_unlock (&self->lock);
  if (!success) {
    GST_ELEMENT_ERROR (
        self, STREAM, FAILED, ("video conversion failed"),
        ("%s", error_message != nullptr ? error_message : "unknown error"));
    g_free (error_message);
...
static void
gst_dsx_video_convert_finalize (GObject *object)
{
  GstDsxVideoConvert *self = GST_DSX_VIDEO_CONVERT (object);
  delete self->backend;
...
static void
gst_dsx_video_convert_class_init (GstDsxVideoConvertClass *klass)
{
  ...
  g_object_class_install_property (
      ...
      g_param_spec_uint ("gpu-id", "GPU ID", "Set GPU Device ID for operation",
                         0, G_MAXUINT, 0,
                         static_cast<GParamFlags> (G_PARAM_READWRITE
                                                   | GST_PARAM_MUTABLE_READY
                                                   | G_PARAM_STATIC_STRINGS)));
  g_object_class_install_property (
      object_class, PROP_OUTPUT_BUFFERS,
      g_param_spec_uint ("output-buffers", "Output-Buffers",
                         "number of output buffers", 1, G_MAXUINT, 4,
                         static_cast<GParamFlags> (G_PARAM_READWRITE
                                                   | GST_PARAM_MUTABLE_PLAYING
                                                   | G_PARAM_STATIC_STRINGS)));
  g_object_class_install_property (
      object_class, PROP_SOURCE_CROP,
      g_param_spec_string ("src-crop", "Src-Crop",
                           "Pixel location left:top:width:height", "0:0:0:0",
                           static_cast<GParamFlags> (
                               G_PARAM_READWRITE | GST_PARAM_MUTABLE_PLAYING
                               | G_PARAM_STATIC_STRINGS)));
  g_object_class_install_property (
      object_class, PROP_DESTINATION_CROP,
      g_param_spec_string (
          "dest-crop", "Dest-Crop", "Pixel location left:top:width:height",
          "0:0:0:0",
          static_cast<GParamFlags> (G_PARAM_READWRITE | GST_PARAM_MUTABLE_READY
                                    | G_PARAM_STATIC_STRINGS)));
  g_object_class_install_property (
      object_class, PROP_BLOCK_LINEAR_OUTPUT,
      g_param_spec_boolean (
          "bl-output", "Blocklinear output",
          "Blocklinear output, applicable only for memory:NVMM NV12 output",
          FALSE,
          static_cast<GParamFlags> (G_PARAM_READWRITE
                                    | G_PARAM_STATIC_STRINGS)));
  g_object_class_install_property (
      object_class, PROP_ALLOW_ODD_CROP,
      g_param_spec_boolean (
          "allow-odd-crop", "Allow Odd Crop",
          "Allow the odd dimensions for source and destination crop rectangle",
          TRUE,
          static_cast<GParamFlags> (G_PARAM_READWRITE
                                    | G_PARAM_STATIC_STRINGS)));
  g_object_class_install_property (
      object_class, PROP_NVBUF_MEMORY_TYPE,
      g_param_spec_enum (
          ...
          static_cast<GParamFlags> (G_PARAM_READWRITE | GST_PARAM_MUTABLE_READY
                                    | G_PARAM_STATIC_STRINGS)));
  g_object_class_install_property (
      object_class, PROP_COMPUTE_HW,
      g_param_spec_enum ("compute-hw", "Compute Scaling HW",
                         "Compute Scaling HW", dsx_compute_hw_get_type (), 0,
                         static_cast<GParamFlags> (G_PARAM_READWRITE
                                                   | GST_PARAM_CONTROLLABLE
                                                   | G_PARAM_STATIC_STRINGS)));
  g_object_class_install_property (
      object_class, PROP_INTERPOLATION_METHOD,
      g_param_spec_enum (
          "interpolation-method", "Interpolation-method",
          "Set interpolation methods", dsx_interpolation_method_get_type (), 6,
          static_cast<GParamFlags> (G_PARAM_READWRITE | GST_PARAM_CONTROLLABLE
                                    | G_PARAM_STATIC_STRINGS)));
  g_object_class_install_property (
      object_class, PROP_COPY_HW,
      g_param_spec_enum (
          "copy-hw", "Copy HW", "Select hardware used for surface copies.",
          dsx_copy_hw_get_type (), 1,
          static_cast<GParamFlags> (G_PARAM_READWRITE | GST_PARAM_CONTROLLABLE
                                    | G_PARAM_STATIC_STRINGS)));
  g_object_class_install_property (
      object_class, PROP_CONTIGUOUS_BUFFERS,
      g_param_spec_boolean (
          "contiguous-buffers", "Contiguous Buffers",
          "Transformed output buffers in a batch are contiguous in memory.",
          FALSE,
          static_cast<GParamFlags> (G_PARAM_READWRITE
                                    | G_PARAM_STATIC_STRINGS)));
  g_object_class_install_property (
      object_class, PROP_DISABLE_PASSTHROUGH,
      g_param_spec_boolean ("disable-passthrough", "Disable Passthrough",
                            "Disable passthrough mode at init time", FALSE,
                            static_cast<GParamFlags> (
                                G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));
  g_object_class_install_property (
      object_class, PROP_FLIP_METHOD,
      g_param_spec_enum ("flip-method", "Video flip methods",
                         "video flip methods", dsx_flip_method_get_type (), 0,
                         static_cast<GParamFlags> (G_PARAM_READWRITE
                                                   | GST_PARAM_CONTROLLABLE
                                                   | G_PARAM_STATIC_STRINGS)));
  ...
  gst_element_class_set_static_metadata (
      element_class, "DSX Video Convert Plugin",
  ...
  transform_class->prepare_output_buffer
      = gst_dsx_video_convert_prepare_output_buffer;
  transform_class->transform = gst_dsx_video_convert_transform;
}

static void
gst_dsx_video_convert_init (GstDsxVideoConvert *self)
{
...
{
  GST_DEBUG_CATEGORY_INIT (gst_dsx_video_convert_debug, "dsxvideoconvert", 0,
                           "DSX video converter");
  return gst_element_register (plugin, "dsxvideoconvert", GST_RANK_PRIMARY,
                               GST_TYPE_DSX_VIDEO_CONVERT);
}

GST_PLUGIN_DEFINE (GST_VERSION_MAJOR, GST_VERSION_MINOR, dsxvideoconvert,
