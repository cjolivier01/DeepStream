...
GstFlowReturn
count_sample (GstElement *, gpointer user_data)
{
...
bool
wait_for_count (const std::atomic<guint> &count, guint minimum)
{
...
main (int argc, char **argv)
{
  ...
  g_object_set (sink, "sync", FALSE, "max-buffers", 0U, "emit-signals", TRUE,
                "enable-last-sample", FALSE, nullptr);
  g_signal_connect (sink, "new-sample", G_CALLBACK (count_sample),
                    &sample_count);
  ...
  for (guint index = 0; index < 3; ++index) {
    ...
    gst_buffer_memset (buffer, 0, static_cast<guint8> (index * 71),
                       input_size);
