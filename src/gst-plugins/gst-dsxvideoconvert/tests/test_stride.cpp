...
using VideoMetaMap = gboolean (*) (GstVideoMeta *, guint, GstMapInfo *,
                                   gpointer *, gint *, GstMapFlags);
...
gboolean
counting_video_meta_map (GstVideoMeta *meta, guint plane, GstMapInfo *info,
...
gboolean
counting_video_meta_unmap (GstVideoMeta *meta, guint plane, GstMapInfo *info)
{
  ++custom_unmap_calls;
...
gboolean
check_output_video_meta (GstBuffer *, GstMeta **meta, gpointer user_data)
{
...
std::uint64_t
checksum (const guint8 *data, gsize size)
{
...
main (int argc, char **argv)
{
  gst_init (&argc, &argv);
  if (argc != 3
      || (std::strcmp (argv[2], "compact") != 0
          && std::strcmp (argv[2], "padded") != 0
          && std::strcmp (argv[2], "custom") != 0
          && std::strcmp (argv[2], "undersized-positive") != 0
  ...
  const bool undersized_positive
      = std::strcmp (argv[2], "undersized-positive") == 0;
  const bool undersized_negative
      = std::strcmp (argv[2], "undersized-negative") == 0;
  const bool expect_rejection = undersized_positive || undersized_negative;
  const gint stride = padded ? 320 * 4 + 64
                             : (expect_rejection
                                    ? (undersized_negative ? -64 : 64)
                                    : 320 * 4);
  const gsize offset = padded ? 37U
                              : (undersized_negative ? 239U * 64U : 0U);
  const gsize size = padded ? offset + static_cast<gsize> (stride) * 240 + 19
                            : 320U * 240U * 4U;
  ...
  for (guint y = 0; y < 240; ++y) {
    guint8 *row = map.data + static_cast<gint64> (offset)
                  + static_cast<gint64> (y) * stride;
  ...
  GstVideoMeta *video_meta = gst_buffer_add_video_meta_full (
      buffer, GST_VIDEO_FRAME_FLAG_NONE, GST_VIDEO_FORMAT_RGBA, 320, 240, 1,
      offsets, strides);
  ...
  gst_buffer_foreach_meta (gst_sample_get_buffer (sample),
                           check_output_video_meta,
                           &output_video_meta_count);
  ...
  g_assert_true (gst_buffer_map (gst_sample_get_buffer (sample), &output_map,
                                 GST_MAP_READ));
  std::printf ("size=%zu,checksum=%016" PRIx64 "\n", output_map.size,
               checksum (output_map.data, output_map.size));
