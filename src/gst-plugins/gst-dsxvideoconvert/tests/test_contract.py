...
def negotiated_src_caps(element: str, nvmm_input: bool, nvmm_output: bool) -> dict[str, str]:
    source = "nvvideotestsrc" if nvmm_input else "videotestsrc"
    input_media = "video/x-raw(memory:NVMM)" if nvmm_input else "video/x-raw"
    output_media = "video/x-raw(memory:NVMM)" if nvmm_output else "video/x-raw"
    input_fields = "format=RGBA,width=320,height=240,framerate=30/1"
    if nvmm_input:
        input_fields += ",batch-size=1,num-surfaces-per-frame=1"
    else:
        input_fields += ",multiview-mode=mono"
    command = [
        "gst-launch-1.0", "-v", source, "num-buffers=1", "!",
        f"{input_media},{input_fields}", "!", element, "!",
        f"{output_media},format=NV12,width=160,height=120", "!",
        "fakesink", "sync=false",
    ]
    output = run(*command)
    marker = f"{element}0.GstPad:src: caps = "
