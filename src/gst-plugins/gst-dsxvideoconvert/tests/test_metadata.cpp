...
bool
link_many (GstElement *first, GstElement *second, GstElement *third,
           GstElement *fourth)
{
  return gst_element_link (first, second) && gst_element_link (second, third)
         && gst_element_link (third, fourth);
}

void
attach_metadata (GstBuffer *buffer)
{
  NvDsBatchMeta *batch = nvds_create_batch_meta (1);
  g_assert_nonnull (batch);
  NvDsMeta *meta = gst_buffer_add_nvds_meta (buffer, batch, nullptr,
...
void
print_metadata (GstBuffer *buffer)
{
  ...
  auto *display
      = static_cast<NvDsDisplayMeta *> (frame->display_meta_list->data);

  std::printf ("frame=%u,%u,%u,%u\n", frame->source_frame_width,
               frame->source_frame_height, frame->pipeline_width,
               frame->pipeline_height);
  std::printf ("object=%.3f,%.3f,%.3f,%.3f,%u\n", object->rect_params.left,
               object->rect_params.top, object->rect_params.width,
               object->rect_params.height, object->rect_params.border_width);
  std::printf ("text=%u,%u,%u\n", object->text_params.x_offset,
               object->text_params.y_offset,
               object->text_params.font_params.font_size);
  g_assert_nonnull (frame->obj_meta_list->next);
  auto *outside_object
      = static_cast<NvDsObjectMeta *> (frame->obj_meta_list->next->data);
  std::printf (
      "outside-object=%.3f,%.3f,%.3f,%.3f,%u\n",
      outside_object->rect_params.left, outside_object->rect_params.top,
      outside_object->rect_params.width, outside_object->rect_params.height,
      outside_object->rect_params.border_width);
  std::printf ("outside-text=%u,%u,%u\n", outside_object->text_params.x_offset,
               outside_object->text_params.y_offset,
               outside_object->text_params.font_params.font_size);
  std::printf ("display=%.3f,%.3f,%.3f,%.3f\n", display->rect_params[0].left,
               display->rect_params[0].top, display->rect_params[0].width,
               display->rect_params[0].height);
  std::printf ("line=%u,%u,%u,%u,%u\n", display->line_params[0].x1,
               display->line_params[0].y1, display->line_params[0].x2,
               display->line_params[0].y2, display->line_params[0].line_width);
  std::printf ("label=%u,%u,%u\n", display->text_params[0].x_offset,
               display->text_params[0].y_offset,
               display->text_params[0].font_params.font_size);
  std::printf ("arrow=%u,%u,%u,%u,%u\n", display->arrow_params[0].x1,
               display->arrow_params[0].y1, display->arrow_params[0].x2,
               display->arrow_params[0].y2,
               display->arrow_params[0].arrow_width);
  std::printf ("circle=%u,%u,%u,%u\n", display->circle_params[0].xc,
               display->circle_params[0].yc, display->circle_params[0].radius,
...
main (int argc, char **argv)
{
  ...
  g_object_set (source, "caps", input_caps, "format", GST_FORMAT_TIME,
                nullptr);
  g_object_set (converter, "src-crop", "20:10:200:100", "dest-crop",
                "5:7:100:50", "flip-method", flip_method, nullptr);
