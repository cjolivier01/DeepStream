...
GstPadProbeReturn
answer_batch_query (GstPad *, GstPadProbeInfo *info, gpointer)
{
...
std::uint64_t
hash_bytes (std::uint64_t hash, const guint8 *data, gsize size)
{
...
std::uint64_t
surface_checksum (NvBufSurface *batch, guint index)
{
  NvBufSurfaceParams &surface = batch->surfaceList[index];
  const bool mapped = batch->memType == NVBUF_MEM_SURFACE_ARRAY
                      || batch->memType == NVBUF_MEM_HANDLE;
  if (mapped) {
    g_assert_cmpint (NvBufSurfaceMap (batch, static_cast<int> (index), -1,
                                     NVBUF_MAP_READ),
                     ==, 0);
    g_assert_cmpint (NvBufSurfaceSyncForCpu (batch, static_cast<int> (index),
                                            -1),
                     ==, 0);
  }
  std::uint64_t hash = UINT64_C (1469598103934665603);
  for (guint plane = 0; plane < surface.planeParams.num_planes; ++plane) {
    const guint row_bytes = surface.planeParams.width[plane]
                             * surface.planeParams.bytesPerPix[plane];
    const guint rows = surface.planeParams.height[plane];
    std::vector<guint8> pixels (static_cast<gsize> (row_bytes) * rows);
    const auto *source
        = mapped ? static_cast<const guint8 *> (surface.mappedAddr.addr[plane])
                 : static_cast<const guint8 *> (surface.dataPtr)
                       + surface.planeParams.offset[plane];
    if (mapped) {
      for (guint row = 0; row < rows; ++row) {
        std::memcpy (pixels.data () + row * row_bytes,
                     source + row * surface.planeParams.pitch[plane],
                     row_bytes);
      }
    } else {
      g_assert_cmpint (
          cudaMemcpy2D (pixels.data (), row_bytes, source,
                        surface.planeParams.pitch[plane], row_bytes, rows,
  ...
  if (mapped) {
    g_assert_cmpint (NvBufSurfaceUnMap (batch, static_cast<int> (index), -1),
                     ==, 0);
  }
  return hash;
}
...
main (int argc, char **argv)
{
  ...
  if ((argc != 3 && argc != 4)
      || (std::strcmp (argv[2], "nvmm") != 0
          && std::strcmp (argv[2], "raw") != 0)
      || (option != nullptr && std::strcmp (option, "caps-only") != 0
          && std::strcmp (option, "contiguous") != 0
          && std::strcmp (option, "block-linear") != 0
          && std::strcmp (option, "compute-gpu") != 0
  ...
  if (NvBufSurfaceMemSet (input_surface, 0, -1, 17) != 0
      || NvBufSurfaceMemSet (input_surface, 1, -1, 203) != 0) {
  ...
  if (pipeline == nullptr || source == nullptr || converter == nullptr
  ...
  gst_bin_add_many (GST_BIN (pipeline), source, converter, filter, sink,
                    nullptr);
  if (option == nullptr || std::strcmp (option, "caps-only") != 0) {
    GstPad *source_pad = gst_element_get_static_pad (source, "src");
    gst_pad_add_probe (source_pad, GST_PAD_PROBE_TYPE_QUERY_UPSTREAM,
                       answer_batch_query, nullptr, nullptr);
  ...
  if (output_nvmm) {
    ...
    std::printf ("batch=%u,filled=%u,gpu=%u,mem=%d,contiguous=%d\n",
                 output_surface->batchSize, output_surface->numFilled,
    ...
    for (guint index = 0; index < output_surface->numFilled; ++index) {
      ...
      std::printf ("surface=%u,%u,%u,%d,%d,checksum=%016" PRIx64 "\n",
                   index, params.width, params.height,
    ...
    for (guint index = 0; index < 2; ++index) {
      ...
      const std::uint64_t checksum
          = hash_bytes (UINT64_C (1469598103934665603),
                        output_map.data + index * frame_size, frame_size);
