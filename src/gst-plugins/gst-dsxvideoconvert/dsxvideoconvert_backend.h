...
public:
  ...
  bool configure (const GstVideoInfo &input_info,
                  const GstVideoInfo &output_info, bool input_nvmm,
                  bool output_nvmm, guint batch_size,
                  const DsxBackendOptions &options, gchar **error_message);

  bool transform (GstBuffer *input, GstBuffer *output,
                  const DsxCropRect &source_crop,
                  const DsxCropRect &destination_crop, gchar **error_message);

private:
  bool ensure_staging_surfaces (guint batch_size, gchar **error_message);
  bool upload_raw (GstBuffer *buffer, NvBufSurface *surface,
                   guint batch_size, gchar **error_message) const;
  bool download_raw (const NvBufSurface *surface, GstBuffer *buffer,
                     guint batch_size, gchar **error_message) const;
